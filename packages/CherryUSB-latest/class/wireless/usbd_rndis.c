...
struct usbd_rndis_cfg_priv {
    ...
    uint8_t *vendor_desc;
                ...
                .media_status = NDIS_MEDIA_STATE_CONNECTED, // NDIS_MEDIA_STATE_DISCONNECTED,
                .mtu = RNDIS_MTU,
                .speed = 0,
                .net_filter = 0,
                .init_state = rndis_uninitialized,
                .mac = { 0x00, 0x00, 0x5E, 0x00, 0x53, 0x01 },
                .vendor_id = 0x00ffffff,
                .vendor_desc = "CherryUSB" };

uint8_t usbd_rndis_int_ep;
...
static const uint8_t NOTIFY_RESPONSE_AVAILABLE[8] = { 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
uint8_t notify_buf[8];
static void rndis_notify_rsp(void)
{
    memcpy( notify_buf, NOTIFY_RESPONSE_AVAILABLE, 8 );
    usbd_ep_start_write(usbd_rndis_int_ep, notify_buf, 8);
}

static int rndis_class_request_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
#if 1
    if ( 0==rndis_encapsulated_cmd_handler(*data, *len) )
    {
        *data = (uint8_t*)rndis_encapsulated_resp_buffer;
        *len = ((rndis_generic_msg_t *)rndis_encapsulated_resp_buffer)->MessageLength;
        return 0;
    }
    else {
        USB_LOG_WRN("EEE:%X\r\n", ((rndis_generic_msg_t *)(*data))->MessageType);
        USB_LOG_WRN("bmRequestType 0x%02x, bRequest 0x%02x, wValue 0x%04x, wIndex 0x%04x, wLength 0x%04x\r\n",
                         setup->bmRequestType,
                         setup->bRequest,
                         setup->wValue,
                         setup->wIndex,
        ...
        {
            rndis_encapsulated_resp_buffer[0] = 0;
            *data = (uint8_t*)rndis_encapsulated_resp_buffer;
            *len = 1;
        }
        else
...
struct usbd_interface *usbd_rndis_alloc_intf(uint8_t int_ep)
{
